...
**`data/members.js` 파일 하나만 고치면 됩니다.** 개발 환경을 따로 갖출 필요 없이 웹 브라우저에서 바로 할 수 있습니다.

### 가장 쉬운 방법 (브라우저에서 5분)

1. 사이트 맨 아래 **「내 프로필 추가하기」** 버튼을 누릅니다.
   ([이 링크](https://github.com/uni-park7491/Basic_test/edit/main/data/members.js)를 눌러도 같습니다.)
2. GitHub 로그인 후 편집 화면이 열리면, 기존 멤버 블록 하나를 통째로 복사해서 아래에 붙여넣고 내 정보로 바꿉니다.
3. 화면 아래 **Commit changes** 를 누릅니다.
   저장소 권한이 없으면 GitHub가 자동으로 *Propose changes* (Pull Request)로 안내해 주니 그대로 제출하면 됩니다.
4. 변경이 `main` 에 반영되면 **1~2분 안에 사이트에 자동 반영**됩니다. 따로 배포 작업은 필요 없습니다.

> 쉼표(`,`)와 중괄호(`{ }`) 짝만 맞으면 됩니다. 실수로 문법이 깨지면 카드가 안 보일 수 있으니,
> 블록을 새로 쓰기보다 **기존 블록을 복사해서 값만 바꾸는 방식**을 권합니다.
